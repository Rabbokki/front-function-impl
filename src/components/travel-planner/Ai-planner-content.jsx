...
export function AIPlannerContent({ destination }) {
  ...
  const handleGenerateItinerary = () => {
    ...
    setTimeout(() => {
      ...
      window.location.href = `/travel-planner/${destination}/step5`;
    }, 3000);
  };
    ...
    <div className="space-y-6">
      <Card className="bg-white p-6 shadow-md">
        ...
        <div className="grid gap-8 md:grid-cols-2">
          <div>
            <div className="mb-6">
              <h3 className="mb-2 text-lg font-medium text-traveling-text">
                여행 선호도
              </h3>
              <Textarea
          ...
          <div className="rounded-lg bg-traveling-background/30 p-6">
            <h3 className="mb-4 text-lg font-medium text-traveling-text">
              AI 추천 일정 예시
            </h3>
            <div className="space-y-4">
              {['1일차', '2일차', '3일차'].map((day, i) => (
                <div key={i} className="rounded-lg bg-white p-4 shadow-sm">
                  <h4 className="font-medium text-traveling-purple">{day}</h4>
            ...
            <p className="mt-4 text-xs text-traveling-text/70">
              * 위 일정은 예시이며, 실제 생성되는 일정은 입력한 선호도에 따라
              달라집니다.
            </p>
          </div>
        </div>
