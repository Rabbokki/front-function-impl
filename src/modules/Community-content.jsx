...
export function CommunityContent() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        // 카테고리와 검색어를 백엔드로 전달하여 필터링된 게시글 목록을 가져옴
        const res = await axios.get('/api/posts', {
          params: {
            category: activeTab,  // 활성화된 탭의 카테고리 전달
            search: searchTerm,    // 검색어 전달
          }
        });
        setPosts(res.data); // 필터링된 게시글 목록을 상태에 저장
      } catch (error) {
        console.error('게시글 불러오기 실패:', error);
      }
    };
    fetchPosts();
  }, [activeTab, searchTerm]); // activeTab이나 searchTerm이 변경될 때마다 호출
